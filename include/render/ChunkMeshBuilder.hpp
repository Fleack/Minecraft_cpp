...
private:
    static void collectVertices(world::Chunk const& chunk, world::World const& world, std::unordered_map<std::string, std::vector<Vertex>>& out);
    ...
    static std::vector<ecs::MeshComponent> buildMeshComponents(std::unordered_map<std::string, std::vector<Vertex>> const& vertsByTex);
